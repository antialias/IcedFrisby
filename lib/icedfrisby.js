...
class Frisby {
  ...
  _request () {
    ...
    console.log(params)
    console.log(this.current.request.json)
    ...
    Object.assign(this._outgoing, outgoing)
  ...
  */
  config (opts) {
    ...
    if ('json' in opts) {
      this.current.request.json = opts.json
    }
    return this
  }
  ...
  get testInfo () {
    const { method, uri } = this._outgoing
    ...
    describeFn(self._message, function () {
      before(function () {
        if (self.current.useApp) {
          ...
          self._outgoing.uri = uri + self._outgoing.uri
        }
      })
      after(function (done) {
  ...
  _makeRequest (done) {
    ...
    const maybePerformRetry = () => {
      ...
      if (this._outgoing.method.toUpperCase() === 'POST') {
        return false
      }
    ...
    const start = cb => {
      ...
      outgoing.json = this.current.request.json
